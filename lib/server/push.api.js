...
var sendWorker = function(task, interval) {
  ...
  return Meteor.setInterval(function() {
    ...
    try {
      ...
      if (Push.debug) {
        console.log('Push: Error while sending 1: ' + error.message);
		console.log(e.stack);
      }
    }
  }, interval);
...
Push.Configure = function(options) {
    ...
    var _querySend = function(query, options) {
      ...
      var countGcm = [];
        ...
        Push.Tokens.find(query).forEach(function(app) {
        ...
        if (Push.debug) {
          ...
          if (!countApn.length && !countGcm.length) {
            if (Push.Tokens.find().count() === 0) {
              console.log('Push, GUIDE: The "Push.Tokens" is empty -' +
                ' No clients have registred on the server yet...');
            }
          } else if (!countApn.length) {
            if (Push.Tokens.find({ 'token.apn': { $exists: true } }).count() === 0) {
              console.log('Push, GUIDE: The "Push.Tokens" - No APN clients have registred on the server yet...');
            }
          } else if (!countGcm.length) {
            if (Push.Tokens.find({ 'token.gcm': { $exists: true } }).count() === 0) {
              console.log('Push, GUIDE: The "Push.Tokens" - No GCM clients have registred on the server yet...');
            }
          }
    ...
    if (options.sendInterval !== null) {
      ...
      sendWorker(function() {
		  ...
		  try {
			  ...
			  console.log('Push: Error while sending 2: ' + e.message);
			  console.log(e.stack);
			  return;
		  }

          if(pendingNotifications instanceof Object) {
              // Set send fence
