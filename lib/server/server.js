Push.Tokens = new Mongo.Collection('push_tokens');
...
Push.addListener('token', function(currentToken, value) {
  if (value) {
	...
	// console.log( stack );
    Push.Tokens.update({ token: currentToken }, { $set: { token: value } }, { multi: true });
  } else if (value === null) {
    // Remove the token for app
	console.log("Remove token");
	// var stack = new Error().stack
	// console.log( stack );
    // Push.Tokens.update({ token: currentToken }, { $unset: { token: true } }, { multi: true });
  }
});
...
Meteor.methods({
  'raix:push-update': function(options) {
    ...
    if (!doc) {
      ...
      Push.Tokens._collection.insert(doc);
    } else {
      // We found the app so update the updatedAt and set the token
	  console.log("raix:push-update: We found the app so update the updatedAt and set the token");
	  console.log(options.token);

      Push.Tokens.update({ _id: doc._id }, {
        $set: {
          updatedAt: new Date(),
          token: options.token
        }
    ...
    if (doc) {
      ...
      var removed = Push.Tokens.remove({
        $and: [
          { _id: { $ne: doc._id } },
  ...
  'raix:push-setuser': function(id) {
    ...
    var found = Push.Tokens.update({ _id: id }, { $set: { userId: this.userId } });
  ...
  'raix:push-metadata': function(data) {
    ...
    // Set the metadata
    var found = Push.Tokens.update({ _id: data.id }, { $set: { metadata: data.metadata } });
  ...
  'raix:push-enable': function(data) {
    ...
    var found = Push.Tokens.update({ _id: data.id }, { $set: { enabled: data.enabled } });
